...
static int parseSeedList(tracker_t *tracker, peer_t *peer, char **filename,
                         char **saveptr, char *response_buffer) {
...
int handleAnnounce(tracker_t *tracker, peer_t *current_peer, char **saveptr,
                   char *response_buffer) {
  char *listen_kw = strtok_r(NULL, " ", saveptr);
  char *port_str = strtok_r(NULL, " ", saveptr);

  if (!listen_kw || !port_str || strcmp(listen_kw, "listen") != 0)
    return -1;
  ...
  if (seed_kw && strcmp(seed_kw, "seed") == 0) {
    if (parseSeedList(tracker, current_peer, &file_name, saveptr,
                      response_buffer) != 0) {
      return -1;
    }
  }
...
static void parseLookCriteria(char **saveptr, char *target_filename,
                              int *target_filesize) {
  char *criterion;
