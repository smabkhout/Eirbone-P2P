...
public class Peer {
  ...
  public byte[] requestPieces(int targetPort, String key,
    ...
    try (Socket socket = new Socket("127.0.0.1", targetPort);
      ...
      System.err.println("Error requesting pieces from localhost:" +
                         targetPort + " - " + e.getMessage());
      return null;
    }
  }
  ...
  public void sendLook(String filename) {
    try {
      if (trackerOut == null || trackerIn == null) {
        System.err.println(
            "Tracker connection is not available. Start the tracker before using look.");
        return;
      }
  ...
  public void sendGetFile(String key) {
    try {
      if (trackerOut == null || trackerIn == null) {
        System.err.println(
            "Tracker connection is not available. Start the tracker before using getfile.");
        return;
      }
