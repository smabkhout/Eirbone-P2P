...
public class PeerDiscovery {
    ...
    // Method to parse the Tracker's response: "peers <hash> [1.1.1.2:2222 1.1.1.3:3333]"
    public void updatePeersFromTracker(String response) {
        // 1. Remove brackets and prefix
        String listPart = response.substring(response.indexOf("[") + 1, response.indexOf("]"));
...
class RemotePeer {
    String ip;
    int port;
    public RemotePeer(String ip, int port) { this.ip = ip; this.port = port; }
}