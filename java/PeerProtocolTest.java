...
public class PeerProtocolTest {
  ...
  private static void testInterestedHaveBetweenTwoPeers() throws Exception {
    ...
    serverPeer.registerFile(key, tempFile.getAbsolutePath(), tempFile.length(),
                            FileState.SEED);
    ...
    assertTrue(response.startsWith("have " + key + " "),
               "Unexpected have response: " + response);
    ...
    assertTrue(decoded.length == 1,
               "Expected 1-byte buffermap, got: " + decoded.length);
    assertTrue((decoded[0] & 0xFF) == 0xC0,
               "Unexpected buffermap value: 0x" +
  ...
  private static void testGetPiecesDataBetweenTwoPeers() throws Exception {
    ...
    assertTrue(have != null && have.startsWith("have " + key + " "),
               "Unexpected HAVE response: " + have);

    byte[] payload =
        clientPeer.requestPieces(serverPort, key, Arrays.asList(0, 1));
    assertTrue(payload != null, "No DATA payload received");
    assertTrue(payload.length == 2048,
               "Payload length mismatch: " + payload.length);

    for (int i = 0; i < payload.length; i++) {
      byte expected = (byte)(i % 256);
      assertTrue(payload[i] == expected,
                 "Unexpected payload byte at index " + i);
    }
